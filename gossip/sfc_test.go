...
import (
	...
	"github.com/galaxy-digital/galaxychain-chain/gossip/contract/driver100"
	"github.com/galaxy-digital/galaxychain-chain/gossip/contract/driverauth100"
	"github.com/galaxy-digital/galaxychain-chain/gossip/contract/netinit100"
	"github.com/galaxy-digital/galaxychain-chain/gossip/contract/sfc100"
	"github.com/galaxy-digital/galaxychain-chain/logger"
	"github.com/galaxy-digital/galaxychain-chain/utils"
)
...
func TestSFC(t *testing.T) {
	...
	_ = true &&
		...
		t.Run("SFC upgrade", func(t *testing.T) {
			...
			rr := env.ApplyBlock(nextEpoch,
				env.Contract(admin, utils.ToGlxy(0), sfc100.ContractBin),
			)
	...
	t.Run("Direct driver", func(t *testing.T) {
		...
		rr := env.ApplyBlock(nextEpoch,
			env.Contract(admin, utils.ToGlxy(0), anyContractBin),
		)
		require.Equal(1, rr.Len())
		require.Equal(types.ReceiptStatusSuccessful, rr[0].Status)
		newImpl := rr[0].ContractAddress
...
func cicleTransfers(t *testing.T, env *testEnv, count uint64) {
	...
	for i := uint64(0); i < count; i++ {
		...
		for i := range txs {
			from := (i)%accounts + 1
			to := (i+1)%accounts + 1
			txs[i] = env.Transfer(from, to, utils.ToGlxy(100))
		}
